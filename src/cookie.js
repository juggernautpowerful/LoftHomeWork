...
function createRowCookie(name, value) {
...
function getCookies() {
    return document.cookie.split(';').reduce((t, s) => Object.assign(t, { [s.split('=')[0]]: s.split('=')[1] }), {});
}
...
function removeCookie(event) {
    if (event.target.nodeName === 'BUTTON') {
        ...
        document.cookie = event.target.parentNode.parentNode.firstChild.innerText+'=; max-age=0';
        tr.remove();
    } 
}

function isMatching(search, chunk) {
